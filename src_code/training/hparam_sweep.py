...
def objective(trial):
    ...
    X_train, u_train, h_train, _, _, scaler, u_encoder = preprocess_data(df_train, is_train=True)
    X_val, u_val, h_val, _, df_derived_val, _, _ = preprocess_data(df_val, scaler=scaler, u_encoder=u_encoder, is_train=False)
    train_dataset = TensorDataset(torch.tensor(X_train, dtype=torch.float32), torch.tensor(u_train, dtype=torch.float32), torch.tensor(h_train, dtype=torch.float32))
    train_loader = DataLoader(train_dataset, batch_size=128, shuffle=True)
    model = iVAE_MetabolicStateModel(beta=beta, lambda_anchor=lam)
    optim = AdamW(model.parameters(), lr=lr)
    for epoch in range(15):
        model.train()
        for x_b, u_b, h_b in train_loader:
            x_hat, mu_q, logvar_q, mu_p, logvar_p, h_hat, z = model(x_b, u_b)
            loss, recon, kl, anchor = model.loss(x_b, u_b, h_b, x_hat, mu_q, logvar_q, mu_p, logvar_p, h_hat)
            optim.zero_grad()
            loss.backward()
            optim.step()
    model.eval()
